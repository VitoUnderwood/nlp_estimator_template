...
class Model(object):
    def __init__(self, config, key_input_ids, val_input_ids, output_ids, beam_width, maximum_iterations, mode,
                 word_vectors=None, pad=0, unk=3, end_token=2, start_token=1):
        """
        描述模型图结构
        <PAD>', '<S>', '<E>'
        :param config: 存放模型结构相关的参数
        :param key_input_ids [batch_size, max_feat_num]
        :param val_input_ids [batch_size, max_feat_num]
        :param output_ids: [batch_size, max_seq_length]
        """
